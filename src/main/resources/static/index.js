angular.module('app', ['ngStorage']).controller('indexController', function ($scope, $http, $localStorage) {
        ...
        $http.post(url, $scope.user)
            .then(function successCallback(response) {
                if (response.data.token) {
                    ...
                    // $http.get('http://localhost:8189/winter/api/v1/products' + $localStorage.springWebGuestCartId + '/merge')
                    //     .then(function successCallback(response) {
                    //
                    //     });
                    // $location.path('/');
    ...
    $scope.removeFromCart = function (productId){
        const url = 'http://localhost:8189/winter/api/v1/cart/remove/'+ productId;
        $http.get(url).then(function (response) {
    ...
    $scope.clearCart = function (){
        const url = 'http://localhost:8189/winter/api/v1/cart/clear';
        $http.get(url).then(function (response) {
            if (response.status==200){
                $scope.loadCart();
            }
